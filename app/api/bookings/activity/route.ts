...
const validateActivityBooking = (data: any): { valid: boolean; error?: string } => {
  // Name validation
  ...
  if (activityDate < today) {
    return { valid: false, error: 'Activity date cannot be in the past' };
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate input
    const validation = validateActivityBooking(data);
    if (!validation.valid) {
      return NextResponse.json(
    ...
    const rowData = [
      ...
      sanitizeInput(data.activityType),
      data.activityDate,
      data.numberOfGuests,
      sanitizeInput(data.timeSlot || ''),
    ...
    return NextResponse.json({ 
      success: true,
      message: 'Activity booking submitted successfully!' 
    });
  } catch (error: any) {
    console.error('Error submitting activity booking:', error);
