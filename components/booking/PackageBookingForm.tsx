...
const PackageBookingForm: React.FC<PackageBookingFormProps> = ({
  ...
  // Extract number of nights from duration string (e.g., "5 Days / 4 Nights" -> 4)
  const getNightsFromDuration = (duration?: string): number | null => {
    if (!duration) return null;
    const match = duration.match(/(\d+)\s*Nights?/i);
    return match ? parseInt(match[1]) : null;
  };

  const nights = getNightsFromDuration(packageDuration);
  ...
  const validate = () => {
    ...
    // Special requests validation (optional but should be sanitized)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sanitizedData = {
        ...
        packageType: sanitizeInput(packageType),
  ...
  if (isSubmitting) {
    return <BookingLoader bookingType="package" />;
  }
              ...
              <div className="text-center mb-6">
                ...
                <div className="inline-block bg-teal-50 border border-teal-100 rounded-lg px-4 py-2 mb-4">
                  <p className="text-sm text-gray-700">
                    <span className="font-semibold text-teal-700">{packageName}</span>
                  </p>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-left flex-1">
                    ...
                    <p className="text-sm text-gray-600 leading-relaxed">
                      Our team will carefully review your booking details and get back to you via email or phone to confirm everything is set up perfectly for your stay.
                    </p>
                  </div>
                </div>
              </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 p-6 flex items-center justify-between z-10 rounded-t-2xl">
          <div>
            <h2 className="text-3xl font-black text-white mb-1">Book {packageName}</h2>
        ...
        <div className="bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50">
          ...
          <div>
            ...
            {nights !== null && (
              <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-sm text-amber-800 font-medium">
                  ✨ <strong>Auto-fill:</strong> When you select a check-in date, the check-out date will automatically be set for <strong>{nights} nights</strong> ({packageDuration}).
                </p>
              </div>
            )}
            <DateRangePicker
              ...
              onStartDateChange={(date) => {
                setFormData(prev => {
                  ...
                  if (date && nights !== null) {
                    ...
                    calculatedEndDate.setDate(calculatedEndDate.getDate() + nights);
                ...
                setErrors(prev => {
                  const newErrors = { ...prev };
                  if (newErrors.startDate) delete newErrors.startDate;
                  if (nights !== null && newErrors.endDate) delete newErrors.endDate;
                  return newErrors;
                });
              }}
            ...
            />
            {errors.startDate && <p className="mt-1.5 text-sm text-red-600 font-medium flex items-center gap-1">
              <span>⚠️</span> {errors.startDate}
            </p>}
            {errors.endDate && <p className="mt-1.5 text-sm text-red-600 font-medium flex items-center gap-1">
...
export default React.memo(PackageBookingForm);
